...
import OtherReportCell from './components/OtherReportCell/index.vue'
import { outerGamesEnum } from '@/01-kk-system/allDefine/enum/frontend/game'
...
let currentDate: { dateFrom: string, dateTo: string, contentCategory: string } = {
...
async function fetchList() {
  ...
  const { dateFrom, dateTo, contentCategory } = currentDate || {}
  const params = {
    walletType,
    currencyCode,
...
<template>
  <Container :title="$t('web.i18nFront.label.personOuterpReport')">
        ...
        <van-pull-refresh v-model="refreshing" class="h-full !overflow-y-scroll" @refresh="onRefresh">
          ...
          >
            <template v-if="list && list.length > 0">
              <OtherReportCell v-for="(data, i) in list" :key="i" :item-data="data" :wallets-list="walletsList" />
            </template>
            <template v-else>
